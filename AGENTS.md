...
- Remote control security hardening: pairing URL の signed proof、request auth v2 の nonce、再起動後も残る replay window、明示的な approval provenance を Host/iOS/Watch 間で共有する。automated verification は `VEQRAL_TEST_MODE=1` と一時 `VEQRAL_HOST_HOME` 内の file-backed secret store を必須とし、実ユーザーの login Keychain・default/search list・LaunchAgent・Hermes credential を変更しない。debug Host は明示 override なしで system Keychain に fallback しない。実 Keychain integration は使い捨て macOS user または VM だけで確認する。
- GitHub Actions CI: `.github/workflows/ci.yml` は `macos-26` / Xcode 26.5 で MacHost unit、isolated security smoke、test-isolation checker、iOS UI test bundle、Mac Catalyst、watchOS build を実行する。実端末、実 Keychain、外部通知、local LLM、production deploy は CI の対象外。
