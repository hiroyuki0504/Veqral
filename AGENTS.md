...
4. #A6 Watch（環境待ち）: watchOS 26.5 platform は取得済みで `VeqralWatch` generic build は PASS。次は iOS target への embed、実 Watch 配布、Tailscale/WebSocket/cellular reachability、APNs capability を順に検証する。現 free team では push は不可。
5. 実機検証（継続）
...
9. Swarm orchestration（GO 待ち）: PR #38〜#41 は stacked Draft PR。Phase 1〜4 は実装・検証済みだが `main` 未統合。レビュー後、ユーザー GO が出たら #38→#41 を直列 integration で取り込み、pre/integration/post-main verify を通す。Phase 5 の PM/委任は将来作業として残す。
