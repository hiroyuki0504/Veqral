...
- #A3 (`codex/a3-cost-governance`): Mac Host に project cost summary / budget API を追加し、usage cost 上限・80%しきい値・auto pause・high severity approval gate を実装。Run 詳細と司令塔 Asset 詳細に「コストガード」を追加。`COST_GOVERNANCE_PR_A3.md` に記録。
...
2. 差別化バックログ（継続）: #A4 司令塔を実データで、が次の未完了項目。
   - #A0/#A1/#A2 は Draft PR 済み。#A3 は `codex/a3-cost-governance` で実装済み、Draft PR 化と検証完了後に次へ進む。
