...
iPhone/iPad から、自分の Mac 上で動く Codex CLI / Claude Code を直接操作するネイティブ Apple クライアント。SwiftUI ネイティブ（iOS + iPad + 最小 Mac Catalyst shell）+ Swift 製の Mac Host。Mac ではターミナルと Mac Host を主に使い、Catalyst UI は状態確認・pairing・承認などの補助に留める。Hermes のオーケストレーション/記憶は Hermes Desktop に委譲する。
...
- Veqral の第一級面は iPhone/iPad の Codex 直接 / Claude 直接。各 CLI の履歴・session は各自 siloed のまま扱う。
- Mac Catalyst はターミナル運用前提の最小 shell。Host 状態、pairing、承認、run 確認などの補助以上に広げない。
...
- `codex/minimal-mac-terminal-shell` (#45): 最終ベース。Mac Catalyst の常時 3 ペイン司令塔 UI を主面から外し、Host 状態、copy できるターミナル起動/ログ確認コマンド、Devices/Approvals/Runs/Memory の最小 sheet だけに縮退。iPhone/iPad の主面は Codex / Claude の2本立て直接クライアントに再配置し、下部 composer から direct run、Codex/Claude app card、ネイティブ履歴導線、drawer navigation、Sales Lab/GitHub/Diff/Artifacts の Run Tools、Projects/Memory/Portfolio の Parked 化を実装。Hermes handoff CTA は主面から外した。Mac Host に `smoke-direct-clients` を追加し、隔離 `CODEX_HOME` / `CLAUDE_CONFIG_DIR` で history list/detail/resumeID、Codex/Claude resume 引数、risk gate を検証する。
- `codex/minimal-mac-terminal-shell` voice follow-up: #42 (`codex/chatgpt-mobile-ux-voice-fix`) は #45 の祖先ではなかったため、実機 crash hardening を #45 に手動で取り込んだ。mic tap は sheet 表示だけ、`VoiceCommandSheet` の `onAppear` 自動録音開始は廃止、permission callback は main actor delivery、iOS 17+ は `AVAudioApplication.requestRecordPermission`、初回 Speech/microphone grant は録音 hardware を起動せず warmup で止める。live `AVAudioEngine.installTap` dictation は外し、Linear PCM `.caf` を `AVAudioRecorder` で録音して停止後に `SFSpeechURLRecognitionRequest` で文字起こしする。録音中は赤ドット/経過時間/入力レベルを表示し、失敗時は「音声エラー」で詰ませず編集可能な Ready fallback に落とす。Gate2 voice XCUITest は iPhone/iPad Simulator で grant / deny / forced error / recording indicator とも PASS。
- `codex/minimal-mac-terminal-shell` distribution status: 最新 #45 build は iPhone 17 Pro Simulator / iPad Pro 13-inch Simulator に install + launch 済み、Mac Catalyst app も launch 済み。実機 iPhone / iPad / Watch は Xcode では Offline、`devicectl` では unavailable、USB 低レベル確認でも未検出だったため、この run では real-device install は未実施。
- Branch convergence: #45 を最終形とし、#42 は voice hardening の供給元として吸収済み、#43 (`codex/direct-clients-reposition`) は #45 に置換、#38〜#41 swarm 系は Veqral direct-client baseline には入れず park/close 方針。

## 未完了・次の手番

1. #45 実機 voice acceptance: iPhone/iPad で mic を tap → sheet が開くだけで落ちない → `録音開始` tap → Speech 許可で落ちない → もう一度 `録音開始` → microphone 許可で落ちない → もう一度 `録音開始` → 赤ドット・経過時間・入力レベルが動く → `停止` → 停止後に文字起こしされる、または文字起こし失敗時に編集可能な Ready fallback へ進むことを確認する。拒否でも crash しないこと。XCUITest は simulator grant/deny/error/recording indicator PASS 済みだが、実マイク音声は人手確認が必要。
2. 実機 direct client acceptance: iPhone/iPad で Mac Host とペアリングし、Codex / Claude app card、History list/detail/resume、新規 direct run、approval sheet、diff/artifacts、Watch approval 維持を確認する。Simulator では fresh install で Codex/Claude 主面、drawer、未ペアリング direct run 待機ログを確認済み。
3. #45 main landing: 1〜2 が OK になりユーザーが明示的に GO したら、退避点作成 → pre-merge verify → main 着地 → post-main full verify → #42/#43/#38〜#41 の中間/重複 PR 整理。force-push はしない。
...
5. Sales Lab follow-up: official Google Places API を使う discovery は、利用規約・quota・API key 管理を確認してから別 PR。現状は手動登録/CSV のみ。実ブラウザ screenshot と提案書テンプレート品質も別 PR で強化。
6. Mac UI follow-up: Mac Catalyst はターミナル shell の補助に留める。Mac 上の新しい操作面が必要になった場合は、まず MacHost CLI / menu bar / iPhone・iPad 側で足りるか確認し、Catalyst の大型 UI を増やさない。
7. Gate1: #0 Hermes 記憶継承は `openai-codex/gpt-5.5 -> openai-codex/gpt-5.4` の real 2 model で PASS 済み。`HERMES_MEMORY_INHERITANCE_PR0.md` に実トランスクリプトあり。自作 memory は足していない。
   - より強いクロスベンダー証明は、Hermes から Claude/Anthropic login が使える状態になった後で再実行する。
8. Gate2（継続）: #A1 XCUITest は iPhone Simulator / iPad Simulator で PASS 済み。実機では `DEVICE_ACCEPTANCE.md` に沿って iPhone/iPad 5項目を確認。
...
9. #A6 Watch（環境待ち）: Xcode に watchOS 26.5 platform を入れた後、`VeqralWatch` を build し、iOS target への embed、実 Watch/Tailscale/WebSocket/cellular reachability、APNs capability を順に検証する。現 free team では push は不可。
10. 実機検証（継続）
...
11. 司令塔 Host 設定: `VEQRAL_PORTFOLIO_CODE_ROOTS` / `VEQRAL_PORTFOLIO_ENGAGEMENT_ROOTS` / registry repo / Discord webhook は Parked 扱い。Hermes Desktop と重複しない用途だけ確認する。
12. push 再有効化：有料 Apple Developer Program 加入後（capability 戻す + flag ON + APNs `.p8`/Key ID/Team ID + Host の env: `VEQRAL_PUSH_ENABLED` 他）
13. UI 磨き：スクショ駆動で Codex / Claude 主面をピンポイント改善（CC Pocket / Supabase の質感、AI くささ排除）
14. 組織化：PM / delegation は Hermes Desktop 側で積む。Veqral 側では direct client の範囲を超えない。
...
- 既定の「やらないこと」: MCP・自作共有メモリ / Mac mini 2台目 / Cron / Gateway / App Store / 内蔵 remote desktop / 設定画面の全面改修 / Mac Catalyst の大型司令塔 UI 再拡張。
