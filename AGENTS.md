...
1. #A7（次）: Claude login 復旧後、`verify-memory-inheritance` を A=Claude / B=GPT で再実走する。現状 Hermes から Claude/Anthropic login は未復旧なので、偽 pass を作らず blocking reason を記録する。
...
3. Gate2（継続）: `DEVICE_ACCEPTANCE.md` と #A1 XCUITest に沿って iPhone/iPad 実機5項目を確認。
