...
4. スタック統合：Gate2 green 後、#28 の計画どおり #9→最新を main にまとめて取り込む。`main` への merge はユーザーの明示承認待ち。
