...
追加 follow-up 3: 実機 iPhone / iPad で `Start Recording` tap でも落ちたため、実機入力へ `AVAudioEngine.installTap` を挿す live dictation 経路を P0 から外した。録音中は `AVAudioRecorder` で一時 `.m4a` に保存し、停止後に `SFSpeechURLRecognitionRequest` で文字起こしする。リアルタイム partial 表示は弱くなるが、実機 hardware format / input tap / engine start の crash 要因を避ける。
...
| 実機 iPhone `iPhone16 Ultra Pro Max` | recorder voice build を free team signing build PASS、install PASS、devicectl launch PASS。実機 XCUITest は `.ui-tests.xctrunner` provisioning 未作成で停止 |
| 実機 iPad `Thinkpad` | recorder voice build を free team signing build PASS、install PASS、devicectl launch PASS。実機 XCUITest は `.ui-tests.xctrunner` provisioning 未作成で停止 |
