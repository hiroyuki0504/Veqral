...
追加 follow-up 2: 実機 iPhone / iPad で「マイクボタンを押した瞬間」に落ちる報告があったため、composer の mic tap は sheet 表示だけに限定した。`VoiceCommandSheet` の `onAppear` 自動録音開始を廃止し、`SFSpeechRecognizer` と `AVAudioEngine` は `Start Recording` を押した後に lazy に生成する。これにより、mic tap 時点では Speech / AVFoundation の permission・hardware 経路に触れない。

追加 follow-up 3: 実機 iPhone / iPad で `Start Recording` tap でも落ちたため、実機入力へ `AVAudioEngine.installTap` を挿す live dictation 経路を P0 から外した。録音中は `AVAudioRecorder` で一時 `.caf` に保存し、停止後に `SFSpeechURLRecognitionRequest` で文字起こしする。リアルタイム partial 表示は弱くなるが、実機 hardware format / input tap / engine start の crash 要因を避ける。

追加 follow-up 4: 実機で「音声エラー」となり文字起こしされず、録音中かどうかも分かりにくい報告があったため、録音中の赤ドット / 経過時間 / 入力レベル表示を追加した。録音ファイルは Speech が扱いやすい Linear PCM `.caf` に変更し、文字起こしは remote → on-device retry、partial 結果の救済、25 秒 timeout を持つ。文字起こしに失敗しても「音声エラー」で閉じず、Ready 状態で整形済み欄へ直接入力できる fallback にした。
...
| 実機 iPhone `iPhone16 Ultra Pro Max` | voice meter / PCM recorder build を free team signing build PASS、install PASS、devicectl launch PASS。実機 XCUITest は `.ui-tests.xctrunner` provisioning 未作成で停止 |
| 実機 iPad `Thinkpad` | voice meter / PCM recorder build を free team signing build PASS、install PASS、devicectl launch PASS。実機 XCUITest は `.ui-tests.xctrunner` provisioning 未作成で停止 |
