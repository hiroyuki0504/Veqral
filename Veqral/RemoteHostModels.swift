...
enum RemotePairingLinkError: Error, LocalizedError, Sendable {
    case invalid(String)
    ...
    init(url: URL) throws {
        ...
        func singleton(_ name: String, required: Bool = false) throws -> String? {
            ...
            guard matches.count <= 1 else {
                throw RemotePairingLinkError.invalid("Duplicate pairing field: \(name)")
            }
            if required, matches.isEmpty {
                throw RemotePairingLinkError.invalid("Missing pairing field: \(name)")
            }
        ...
        if let rawVersion = try singleton("pv") {
            ...
            guard (1...8).contains(candidates.count),
                  candidates.first == primary,
            ...
            let auth = try values("auth").map {
                guard let version = Int($0) else {
                    throw RemotePairingLinkError.invalid("Invalid auth version.")
                }
                return version
            }
            let caps = values("cap")
...
struct RemoteHostConfiguration: Codable, Equatable, Sendable {
    ...
    var isPaired: Bool {
        isEnabled && !endpoint.isEmpty && !deviceID.isEmpty && !token.isEmpty
    }
}
