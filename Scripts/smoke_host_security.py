    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="veqral-security-host-") as home:
            ...
            env.update({
                ...
                "VEQRAL_HOST_WORKING_DIRECTORY": str(workspace),
                "VEQRAL_TEST_MODE": "1",
            ...
            # Persist an explicit input request on the approval-gated Run. The Host is
            # restarted so the route test exercises the same server-authoritative state
            # a second paired client could observe, rather than a client-side cache.
            host.terminate()
            host.wait(timeout=5)
            runs_path = Path(home) / "runs.json"
            persisted_runs = json.loads(runs_path.read_text())
            persisted_run = next(item for item in persisted_runs if item["id"] == run_id)
            persisted_run["interaction"] = {
                "kind": "message",
                "prompt": "Choose a deployment target before continuing.",
                "choices": [],
            }
            runs_path.write_text(json.dumps(persisted_runs))
            host = subprocess.Popen([str(BINARY)], env=env, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
            for _ in range(100):
                try:
                    status, health = request(base + "/v1/health")
                ...
                raise AssertionError("isolated Host did not restart with interaction")

            approve_path = f"/v1/runs/{run_id}/approve"
            status, error = request(base + approve_path, "POST", None,
                                    auth_headers(device, token, "POST", approve_path, None, unique_nonce("interaction-approve")))
            assert status == 409 and "input" in error.get("error", "").lower(), (status, error)
            reject_path = f"/v1/runs/{run_id}/reject"
            status, error = request(base + reject_path, "POST", None,
                                    auth_headers(device, token, "POST", reject_path, None, unique_nonce("interaction-reject")))
            assert status == 409 and "input" in error.get("error", "").lower(), (status, error)
            status, snapshot = request(base + snapshot_path, "GET", None,
                                       auth_headers(device, token, "GET", snapshot_path, None))
            assert status == 200 and snapshot["run"].get("interaction", {}).get("prompt"), (status, snapshot)

            # Restore the pure approval fixture for the existing provenance flow.
            host.terminate()
            host.wait(timeout=5)
            persisted_runs = json.loads(runs_path.read_text())
            persisted_run = next(item for item in persisted_runs if item["id"] == run_id)
            persisted_run["interaction"] = None
            runs_path.write_text(json.dumps(persisted_runs))
            host = subprocess.Popen([str(BINARY)], env=env, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
            for _ in range(100):
                try:
                ...
                raise AssertionError("isolated Host did not restart after interaction fixture")
            ...
            safe_input_path = f"/v1/runs/{safe_run_id}/input"
            safe_input_body = {
                "expectedTaskID": safe_snapshot["run"]["taskID"],
                "text": "safe-unsolicited-input-probe",
                "submit": True,
            }
            status, error = request(
                base + safe_input_path,
                "POST",
                safe_input_body,
                auth_headers(
                    device,
                    ...
                    safe_input_path,
                    safe_input_body,
                    unique_nonce("unsolicited-input"),
                ),
            )
            assert status == 400 and "not requesting input" in error.get("error", "").lower(), (status, error)
            ...
            print("PASS: security guards signed-pair=1 auth-v1-downgrade=0 nonce=1 replay=1 restart-replay=1 approval-resume-bypass=0 cancel-resume-bypass=0 unsolicited-input=0 cancel-persistence-process=0 isolated-secret-store=1 cleanup=1")
    finally:
