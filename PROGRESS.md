...
Clean main baseline. #A0〜#A7 were union-integrated after explicit user GO via `codex/final-a-union-20260602`. Swarm orchestration Phase 1〜4 are now stacked Draft PRs from clean main; main merge remains GO-gated.
...
- [x] Phase 1 worktree 並列ランナー — Draft PR #38 (`codex/swarm-orchestration-phase1`), Host worktree isolation, task ledger, scheduler, cancel/kill APIs, verify, commit, optional push/Draft PR, and `smoke-swarm-runner`.
- [x] Phase 2 群制御ボード UI — Draft PR #39 (`codex/swarm-orchestration-phase2`), `群制御` screen, task enqueue/cancel/kill controls, per-agent logs, status overview, and PR links.
- [x] Phase 3 衝突管理 + 資源適応 — Draft PR #40 (`codex/swarm-orchestration-phase3`), scope-hint serialization, Xcode build slot limiting, thermal/load adaptive slots, and expanded parallel smoke coverage.
- [x] Phase 4 直列・GO 制統合準備 — Draft PR #41 (`codex/swarm-orchestration-phase4`), serial integration worktree endpoint/smoke that prepares an integration branch without touching main, plus approval priority/batch controls that keep high-risk review intact.
...
- Swarm orchestration: PR #38〜#41 are stacked Draft PRs (`phase2` on `phase1`, `phase3` on `phase2`, `phase4` on `phase3`). Phase 4 can prepare integration branches and verify them, but it never merges/pushes `main`; final main landing still requires explicit user GO.
