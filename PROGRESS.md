...
Audit/differentiation backlog #A7: Claude login 復旧後の cross-vendor Hermes memory inheritance re-run.
...
- #A6 Apple Watch approval scaffold is documented in `WATCH_APPROVAL_PR_A6.md`. `VeqralWatch` target/scheme exists and the main iOS/Mac builds remain green, but local Watch compilation is blocked by missing watchOS 26.5 platform. APNs remains unavailable under the current free-team setup.
